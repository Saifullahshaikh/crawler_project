...
# #     failed_urls = set()
# #     max_memory_mb = 1000  # Memory threshold (1 GB)
# #     max_pages = 1000  # Maximum number of paginated pages to prevent infinite loops
...
# #         def process_url(current_url):
# #             if current_url in visited or current_url in paginated_urls or current_url in failed_urls:
# #                 logging.debug(f"Skipping URL: {current_url} (already visited, paginated, or failed)")
# #                 return []
# #             visited.add(current_url)
# #             parsed_url = urlparse(current_url)
# #             normalized_path = parsed_url.path.lower()
# #             if any(keyword in normalized_path.split('/') for keyword in ignored_keywords):
# #                 logging.debug(f"Skipping URL: {current_url} (contains ignored keyword)")
# #                 return []
...
# #                     urljoin(current_url, href) not in failed_urls
# #                 ]
# #                 logging.debug(f"Found {len(new_urls)} new URLs to crawl from {current_url}")
# #                 return new_urls
# #             except Exception as e:
# #                 logging.error(f"Error processing {current_url}: {e}")
# #                 return []
...
# #         while not pagination_ended and page <= max_pages:
...
# #                         logging.error(f"Max retries reached for {paginated_url}. Marking as failed.")
# #                         failed_urls.add(paginated_url)
# #                         logging.debug(f"Failed URLs: {failed_urls}")
# #                         page += 1
# #                         break
# #                     time.sleep(2 ** attempt)
# #                 finally:
...
# #                             logging.info("Pruned visited set to reduce memory")
# #             if pagination_ended:
# #                 logging.info("Pagination loop terminated.")
# #                 break
# #             time.sleep(0.2)
...
#                 return new_urls
#             except requests.exceptions.RequestException as e:
#                 logging.error(f"Network error processing {current_url}: {e}")
...
#                     logging.warning(f"Attempt {attempt + 1} failed for {paginated_url}: {e}")
#                     if attempt + 1 == max_retries:
#                         logging.error(f"Network error: Max retries reached for {paginated_url}. Marking as failed.")
#                         failed_urls.add(paginated_url)
#                         logging.debug(f"Failed URLs: {failed_urls}")
#                         page += 1
#                         break
...
#         # Parallel recursive crawling with proper shutdown
#         logging.info("Starting recursive link crawling...")
#         with ThreadPoolExecutor(max_workers=max_workers) as executor:
#             try:
#                 while to_visit:
...
#                     to_visit = to_visit[len(batch):]
#                     futures = [executor.submit(process_url, url) for url in batch]
#                     for future in as_completed(futures, timeout=30):  # Add timeout for each future
#                         try:
#                             new_urls = future.result()
#                             to_visit.extend(new_urls)
#                         except TimeoutError:
#                             logging.error(f"Timeout processing URL in batch: {batch}")
#                         except Exception as e:
...
#                 executor.shutdown(wait=True)  # Ensure all tasks complete before shutdown
...
def crawl_links_recursively(base_url, job_id=None, max_workers=3):
    # Set signal handlers only if in main thread
    if threading.main_thread() == threading.current_thread():
        try:
            signal.signal(signal.SIGINT, signal_handler)
            signal.signal(signal.SIGTERM, signal_handler)
            logging.info("Signal handlers set in main thread")
        except ValueError as e:
            logging.warning(f"Could not set signal handlers: {e}")
    else:
        logging.info("Skipping signal handlers setup (not in main thread)")
